  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center">
          ...
          {grid.map((row, rowIdx) => (
            <div className="flex" key={rowIdx}>
              {row.map((color, colIdx) => (
                <button
                  key={colIdx}
                  onMouseDown={() => handleDown(rowIdx, colIdx)}
                  onMouseEnter={() => handleEnter(rowIdx, colIdx)}
                  className={`w-7 h-7 border ${color}`}
                />
              ))}
            </div>
          ))}
        ...
        <div className="flex flex-wrap justify-center max-w-[1232px]">
          {colors.map((color) => (
            <button
              key={color}
